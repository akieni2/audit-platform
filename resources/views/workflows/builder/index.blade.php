<x-app-layout>
    <div class="mx-auto max-w-7xl space-y-8 px-0 py-2">
        ...
        <div class="flex flex-wrap items-end justify-between gap-4">
            ...
            <a href="{{ route('workflow-builder.create') }}" class="dgcpt-btn-primary">Nouveau workflow</a>
        </div>
        ...
        <div class="grid gap-6 xl:grid-cols-[1.05fr,1.95fr]">
            <div class="dgcpt-surface space-y-4 p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('workflow-builder.store') }}" class="space-y-4">
                    @csrf
                    <div>
                    ...
                    <div>
                        ...
                        <input name="code" type="text" value="{{ old('code') }}" class="dgcpt-input font-mono text-sm" placeholder="WF_DEPT" />
                    </div>
                    <div>
                    ...
                    <button type="submit" class="dgcpt-btn-primary">Créer le workflow</button>
                </form>
            </div>
                    ...
                    <div class="dgcpt-surface p-6 shadow-sm">
                            ...
                            <div>
                                <div class="flex flex-wrap items-center gap-2">
                                    <span @class([
                                        ...
                                        'bg-[#123D2C] text-[#7EF2BE]' => ($template->status?->value ?? $template->status) === \App\Models\WorkflowTemplate::STATUS_PUBLISHED,
                                        'bg-[#2A2140] text-[#C9AEFF]' => ($template->status?->value ?? $template->status) === \App\Models\WorkflowTemplate::STATUS_DRAFT,
                                        'bg-[#4A3314] text-[#FFD479]' => ($template->status?->value ?? $template->status) === \App\Models\WorkflowTemplate::STATUS_DEPRECATED,
                                        'bg-[#3A1A20] text-[#FFB4B4]' => ($template->status?->value ?? $template->status) === \App\Models\WorkflowTemplate::STATUS_ARCHIVED,
                                    ])>
                                    ...
                                    @if ($template->is_system)
                                        <span class="rounded-full bg-[#173050] px-2.5 py-1 text-xs font-semibold text-[#73D8FF]">Système</span>
                                    @endif
                                </div>
                        ...
                        <div class="mt-5 grid gap-3 md:grid-cols-4">
                            <div class="rounded-2xl border border-[rgba(0,209,255,0.12)] bg-[rgba(5,8,22,0.7)] p-4">
                                <p class="text-xs font-semibold uppercase tracking-wide text-[#73D8FF]">Stages</p>
                                <p class="mt-2 text-2xl font-bold text-[#E6EEF8]">{{ $template->stages_count }}</p>
                            </div>
                            <div class="rounded-2xl border border-[rgba(0,209,255,0.12)] bg-[rgba(5,8,22,0.7)] p-4">
                                <p class="text-xs font-semibold uppercase tracking-wide text-[#73D8FF]">Transitions</p>
                                <p class="mt-2 text-2xl font-bold text-[#E6EEF8]">{{ $template->transitions_count }}</p>
                            </div>
                            <div class="rounded-2xl border border-[rgba(0,209,255,0.12)] bg-[rgba(5,8,22,0.7)] p-4">
                                <p class="text-xs font-semibold uppercase tracking-wide text-[#73D8FF]">Instances</p>
                                <p class="mt-2 text-2xl font-bold text-[#E6EEF8]">{{ $template->instances_count }}</p>
                            </div>
                            <div class="rounded-2xl border border-[rgba(0,209,255,0.12)] bg-[rgba(5,8,22,0.7)] p-4">
